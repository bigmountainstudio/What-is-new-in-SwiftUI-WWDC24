...
@Bryan asked:
How do we maintain backward compatibility in the same code base with a Core Data app? Not everyone updates on day one.


@Rishi V (Apple) answered:
SwiftData is iOS 17+ and can coexist with Core Data for backward compatibility.  Developers can explore using SwiftData to implement new features for future releases
...
@Rishi V (Apple) answered:
Depending on your backward compatibility goals, SwiftData is iOS 17+ and Core Data goes back many releases.  You will most likely want to coexist for a few releases
...
How does one ensure their entity version hash equivalent? Sorry if that’s a dumb question. Is it just maintaining the exact same set of attributes, relationships, and properties in the model for an entity?
...
Yes, using a shared app container, the extension & your main app can write to the same database.
...
@Rishi V (Apple) answered:
There is no need to manually mirror, setting up a ModelContainer with a ModelConfiguration noting your CK Entitlement will set up a persistent backend with CK syncing enabled!

Marco
Ok, there is a session. Where do you explain how to do that so I can see?
...
@Amit G (Apple) answered:
you can define your Model Attribute with the option @Attribute (.encrypt) to encrypt your property.
...
@David S (Apple)
 answered:
You should be using the traditional data protection levels for encryption at rest on the device. You can use .encrypt for encryption in CloudKit.
...
@Dave N (Apple)
 answered:
FetchDescriptor has the option to set `fetchLimit` & `fetchOffset` so you can page the data in chunks.
...
@Yuhan
 asked:
Currently, I’m using a custom store that conforms NSIncrementalStore protocol as the underlying storage instead of the default SQLite. However, I browsed the SwiftData API documentation, noticed that there doesn't seem to be an option that can be used to specify the store type.
...
@Matthew
 asked:
Can a swiftData model be serialized or stored in some format that I can just restore when the user loads the app for the first time?
...
@Renan
 asked:
Does SwiftData support CloudKit sharing with ShareLink and Transferable? If not, can we still achieve this by having some sort of CoreData code on the side? If so, is there any sample code for this?
2 replies
...
@Scott P (Apple)
 answered:
No. The nature of @Model types requires reference semantics. Consider Self-referential relationships and how the context would keep track of (and propagate) changes to the objects that it's responsible for.
...
@Amit G (Apple)
 answered:
To Integrate CloudKit, you can enable "CloudKit" option while creating the project Or For your target, Capability adds support for iCloud & CloudKit with a container.
...
@Yannik
 asked:
I have the following two entities referring to each other:
```
@Model class Item {
   ...
...
@Duncan
 asked:
Core Data has added new capabilities this year, such as new ways of handling migrations. Are these new capabilities also all available in SwiftData? (I realize this may be covered in a video—I have not been able to view all of them yet.)
1 reply
...
@Keith
 asked:
What happens when I have a view observing a model object that is deleted elsewhere? Is there a way to detect the object has been deleted? The trips sample code crashes in this situation with two windows open on an iPad (see FB12272702). One window shows a list of trips, another window shows trip detail. After deleting a trip from the list, the app crashes when changing the now-deleted trip in detail view.
:eyes:
1
...
Keith
OK, that could work, Will give that a go as a workaround and update the feedback with the results.

----

@Keith
 asked:
Do predicates support complex expressions? For example, in Core Data I often use `NSExpression` to fetch the min and max values of a field.
:eyes:
1
